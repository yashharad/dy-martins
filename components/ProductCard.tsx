...
export const ProductCard: React.FC<ProductCardProps> = ({ product, onViewDetails, onPreview }) => {
  ...
  return (
    <div className="bg-white dark:bg-[#111114] rounded-3xl border border-gray-100 dark:border-white/5 overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 group flex flex-col h-full cursor-pointer relative">
      <div onClick={() => onViewDetails(product)} className="relative aspect-[4/5] overflow-hidden bg-gray-50 dark:bg-[#0c0c0e]">
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-4 group-hover:translate-y-0">
          <button onClick={(e) => { e.stopPropagation(); onPreview(product.image); }} className="bg-white/10 backdrop-blur-xl border border-white/20 px-6 py-3 rounded-2xl flex items-center gap-3 hover:bg-amber-600 hover:border-amber-400 transition-colors shadow-2xl">
      ...
      <div onClick={() => onViewDetails(product)} className="p-6 flex flex-col flex-1">
        <h3 className="font-bold text-base text-gray-900 dark:text-white mb-2 leading-tight group-hover:text-amber-600 transition-colors line-clamp-2">{product.name}</h3>
